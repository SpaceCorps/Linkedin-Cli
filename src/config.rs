...
pub struct AccountConfig {
    /// Apify username if known.
    pub username: String,
    /// Whatever `users/me` called this key when it was added - an email, a username, or id.
    pub identity: String,
    pub added_at: String,
}
...
pub fn config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("LINKEDIN_CONFIG_DIR").filter(|v| !v.is_empty()) {
        return PathBuf::from(dir);
    }

    #[cfg(windows)]
    let dir = std::env::var_os("APPDATA").map(PathBuf::from).unwrap_or_default().join("linkedin-cli");

    #[cfg(not(windows))]
    let dir = {
        let home = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_default();
        if cfg!(target_os = "macos") {
            home.join("Library").join("Application Support").join("linkedin-cli")
        } else {
...
pub fn load() -> Result<Config> {
    ...
    serde_norway::from_str::<Option<Config>>(&text).map(Option::unwrap_or_default).map_err(|e| {
        Error::other(format!("{} is not valid.", path.display()))
            .detail(e.to_string())
            .fix(format!("Fix or delete {}, then run: linkedin login", path.display()))
    })
}
        ...
        match file.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                return Err(Error::other(format!(
                    "Timed out waiting for the lock at {}. Another linkedin process may be stuck.",
                    path.display()
                )));
            }
...
mod tests {
    ...
    fn reads_config_layout() {
        ...
        let (name, acct) = c.find("default").unwrap();
        assert_eq!(name, "default");
        assert_eq!(acct.identity, "jane@example.com");
        assert_eq!(acct.username, "janedoe");
    }
}
